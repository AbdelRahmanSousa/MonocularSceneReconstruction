...
def compute_error_img(metric, img, ref):
	...
	if metric == "MAE":
		...
		ref_srgb = np.clip(color_space_transform(ref, "linrgb2srgb"), 0, 1)
		img_srgb = np.clip(color_space_transform(img, "linrgb2srgb"), 0, 1)
		result = compute_flip(HWCtoCHW(ref_srgb), HWCtoCHW(img_srgb), pixels_per_degree)
		assert np.isfinite(result).all()
		return CHWtoHWC(result)
